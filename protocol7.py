...
while True:
    ...
    for currentItem in urlList:
        ...
        if enableDashboard == '1':
            try:
                writeDataToFile(f'{dashboardTempFolder}{currentItem}.html','<HTML><HEAD><TITLE>' + str(currentItem) + '</TITLE><BODY><H1>' + str(urlList[currentItem]["appname"]) + '}</H1><H2>' + str(urlList[currentItem]["customer"]) + '</H2><P><UL><LI><b>URL</b>: ' + str(urlList[currentItem]["url"]) + '</LI><LI><b>Payload</b>: ' + str(urlList[currentItem]["payload"]) + '</LI><LI><b>Failure history (0=OK, 1=KO)</b>: ' + str(urlList[currentItem]["failure_history"]) + '</LI><LI><b>Orange since</b>: ' + str(urlList[currentItem]["orange_since"]) + '</LI><LI><b>Red since</b>: ' + str(urlList[currentItem]["red_since"]) + '</LI></UL></P><P>' + newLogLine + '</P></BODY></HEAD></HTML>','Advanced analytics file updated successfully','Advanced analytics file refresh FAILED', 'overwrite')
            except Exception as e:
